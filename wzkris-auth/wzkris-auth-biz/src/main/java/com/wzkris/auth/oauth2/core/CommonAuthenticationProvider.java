...
public abstract class CommonAuthenticationProvider<T extends CommonAuthenticationToken>
    ...
    protected final OAuth2ClientAuthenticationToken getAuthenticatedClientElseThrowInvalidClient(
     ...
     */
    @Nonnull
    protected final Set<String> checkClient(
            CommonAuthenticationToken authenticationToken, RegisteredClient registeredClient) {
        if (registeredClient == null) {
            OAuth2ExceptionUtil.throwErrorI18n(
                    BizCode.BAD_REQUEST.value(), OAuth2ErrorCodes.UNSUPPORTED_GRANT_TYPE, "oauth2.client.invalid");
        }

        if (!registeredClient.getAuthorizationGrantTypes().contains(authenticationToken.getGrantType())) {
            OAuth2ExceptionUtil.throwErrorI18n(
                    BizCode.BAD_REQUEST.value(), OAuth2ErrorCodes.UNSUPPORTED_GRANT_TYPE, "oauth2.unsupport.granttype");
        }
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        OAuth2ClientAuthenticationToken clientPrincipal =
                this.getAuthenticatedClientElseThrowInvalidClient(commonAuthenticationToken);
