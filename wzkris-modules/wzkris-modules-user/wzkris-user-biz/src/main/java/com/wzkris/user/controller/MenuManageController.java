...
import com.wzkris.user.domain.req.MenuQueryReq;
import com.wzkris.user.domain.req.MenuReq;
...
public class MenuManageController extends BaseController {
    ...
    public Result<List<MenuInfoDO>> list(MenuQueryReq queryReq) {
        List<MenuInfoDO> menus = menuInfoMapper.selectList(this.buildQueryWrapper(queryReq));
        return ok(menus);
    }

    private LambdaQueryWrapper<MenuInfoDO> buildQueryWrapper(MenuQueryReq queryReq) {
    ...
    public Result<Void> add(@Validated @RequestBody MenuReq req) {
    ...
    public Result<Void> edit(@Validated @RequestBody MenuReq req) {
        if (StringUtil.equalsAny(req.getMenuType(), MenuConstants.TYPE_INNERLINK, MenuConstants.TYPE_OUTLINK)
                && !StringUtil.ishttp(req.getPath())) {
