...
public class RoleManageController extends BaseController {
    ...
    public Result<Page<RoleInfoDO>> listPage(RoleManageQueryReq req) {
        startPage();
        List<RoleInfoDO> list = roleInfoDataScopeManager.list(this.buildQueryWrapper(req));
        return getDataTable(list);
    }

    private LambdaQueryWrapper<RoleInfoDO> buildQueryWrapper(RoleManageQueryReq req) {
        return new LambdaQueryWrapper<RoleInfoDO>()
                .like(StringUtil.isNotEmpty(req.getRoleName()), RoleInfoDO::getRoleName, req.getRoleName())
                .eq(StringUtil.isNotEmpty(req.getStatus()), RoleInfoDO::getStatus, req.getStatus())
