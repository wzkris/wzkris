...
public interface SysDeptMapper extends BaseMapperPlus<SysDept> {
     ...
     * @return 部门列表
     */
    @Select("SELECT dept_id FROM sys_dept WHERE #{deptId} = ANY(ancestors) OR dept_id = #{deptId}")
     ...
     * @return 子部门数
     */
    @Select("SELECT EXISTS(SELECT dept_id FROM sys_dept WHERE #{deptId} = ANY(ancestors) AND status = '0')")
     ...
     * @return 结果
     */
    @Select("SELECT EXISTS(SELECT dept_id FROM sys_dept WHERE parent_id = #{deptId})")
     ...
     * @return 结果
     */
    @Select("SELECT EXISTS(SELECT dept_id FROM sys_user WHERE dept_id = #{deptId})")
