...
INSERT INTO `config_info` VALUES (12, 'common.yml', 'COMMON_GROUP', 'server:\n  # 优雅停机\n  shutdown: graceful\n  # 服务器配置\n  tomcat:\n    accept-count: 100\n    threads:\n      max: 1000\n      min-spare: 200\n    connection-timeout: 10000\n    # 监控\n    mbeanregistry:\n      enabled: true\n\nspring:\n  task:\n    execution:\n      thread-name-prefix: async-threadPool-\n      pool:\n        core-size: 15\n        max-size: 30\n        queue-capacity: 1000\n        keep-alive: 120s\n        allow-core-thread-timeout: false\n      shutdown:\n        await-termination: true\n        await-termination-period: 60s\n    scheduling:\n      thread-name-prefix: scheduling-threadPool-\n      pool:\n        size: 10\n      shutdown:\n        await-termination: true\n        await-termination-period: 60s\n  jackson:\n    time-zone: GMT+8\n    # 日期格式化\n    date-format: yyyy-MM-dd HH:mm:ss\n    serialization:\n      # 格式化输出\n      INDENT_OUTPUT: false\n      # 忽略空Bean转json的错误\n      FAIL_ON_EMPTY_BEANS: false\n      # 关闭日期转换成时间戳\n      WRITE_DATES_AS_TIMESTAMPS: false\n    # 设置空如何序列化\n    defaultPropertyInclusion: ALWAYS\n    deserialization:\n      #json中不存在的属性就报错\n      fail_on_unknown_properties: false\n    parser:\n      # 允许使用无引号字段\n      ALLOW_UNQUOTED_FIELD_NAMES: true\n      # 忽略未定义的属性\n      IGNORE_UNDEFINED: true\n      # 忽略json最后的逗号\n      ALLOW_TRAILING_COMMA: true\n      # 允许反斜杠\n      ALLOW_BACKSLASH_ESCAPING_ANY_CHARACTER: true\n      # 允许出现特殊字符和转义符\n      ALLOW_UNQUOTED_CONTROL_CHARS: true\n      # 允许出现单引号\n      ALLOW_SINGLE_QUOTES: true\n      # 是否允许使用注释\n      ALLOW_COMMENTS: true\n    mapper:\n      # 使用getter取代setter探测属性，如类中含getName()但不包含name属性与setName()，传输的vo json格式模板中依旧含name属性\n      USE_GETTERS_AS_SETTERS: true\n  mvc:\n    # mvc找不到对应处理器则抛出异常\n    throw-exception-if-no-handler-found: true\n    # 关闭DispatcherServlet懒加载\n    servlet:\n      load-on-startup: 0\n  messages:\n    # 国际化资源文件路径\n    basename: i18n/common\n\n#分布式事务\nseata:\n  enabled: false\n  # Seata 应用编号，默认为 ${spring.application.name}\n  application-id: ${spring.application.name}\n  # Seata 事务组编号，用于 TC 集群名\n  tx-service-group: ${spring.application.name}-group\n  # 关闭自动代理\n  enable-auto-data-source-proxy: false\n  # 服务配置项\n  service:\n    # 虚拟组和分组的映射\n    vgroup-mapping:\n      ruoyi-system-group: default\n    # 分组和 Seata 服务的映射\n    grouplist:\n      default: 127.0.0.1:8091\n  config:\n    type: nacos\n    nacos:\n      serverAddr: ${spring.cloud.nacos.server-addr}\n      username: ${spring.cloud.nacos.username}\n      password: ${spring.cloud.nacos.password}\n      group: SEATA_GROUP\n      namespace: application-dev\n  registry:\n    type: nacos\n    nacos:\n      application: seata-server\n      server-addr: ${spring.cloud.nacos.server-addr}\n      username: ${spring.cloud.nacos.username}\n      password: ${spring.cloud.nacos.password}\n      namespace: application-dev\n\n# 暴露监控端点\nmanagement:\n  endpoints:\n    web:\n      exposure:\n        include: \"*\"\n  endpoint:\n    env:\n      access: read_only\n      show-values: always\n    configprops:\n      access: read_only\n      show-values: always\n    beans:\n      access: read_only\n\n# 请求签名\nrequest-signature:\n  keys:\n    wzkris-gateway:\n      secret: my-super-secret-key\n    wzkris-principal:\n      secret: my-super-secret-key-0\n    wzkris-auth:\n      secret: my-super-secret-key-1\n    wzkris-message:\n      secret: my-super-secret-key-2\n    wzkris-monitor-admin:\n      secret: my-super-secret-key-3\n\n# 路由策略\nroute-policy:\n  status: 1\n  defaultHint: \"\"\n  forceHint: 0.1', 'e0c5eaff2a813ffc5565be5d65ec599d', '2023-06-19 02:28:00', '2025-12-08 10:35:08', NULL, '0:0:0:0:0:0:0:1', '', 'application-dev', '公共配置', '', '', 'yaml', '', '');
